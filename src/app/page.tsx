...
export default function Home() {
  ...
  return (
    <div className={screen === "intro" ? styles.landingShell : styles.shell}>
      {screen !== "intro" ? <nav className={styles.nav} aria-label="Medula Navigation"><Image className={styles.brandIcon} src="/assets/medula-logo-icon-video.png" alt="" width={512} height={512} priority /><span className={styles.brandWordmark}>MEDULA</span></nav> : null}
      <main className={styles.main} aria-live="polite">
        {screen === "intro" ? <section className={styles.landingScreen} aria-labelledby="intro-title"><div className={styles.landingCopy}><p className={styles.landingBrand}>medula</p><h1 className={styles.landingTitle} id="intro-title">Medula App</h1><p className={styles.landingSubtitle}>Teste jetzt dein Medizinwissen</p></div><div className={styles.landingDevice} aria-hidden="true"><div className={styles.deviceSpeaker} /><Image src={appPreviewUrl} alt="" width={1179} height={2556} priority /></div><button className={styles.landingButton} type="button" onClick={startQuiz}>Quiz starten</button></section> : null}

        {screen === "subject" ? <section className={`${styles.screen} ${styles.centerScreen} ${styles.subjectScreen}`} aria-labelledby="subject-title"><h2 className={styles.subjectTitle} id="subject-title">Wähle dein Fach.</h2><p className={`${styles.lead} ${styles.subjectLead}`}>Wir geben dir eine passende Frage mit kurzer Erklärung.</p><div className={`${styles.choiceGrid} ${styles.subjectGrid}`} role="group" aria-label="Fach auswählen">{subjectOptions.map((option) => <button className={styles.choice} type="button" key={option.label} onClick={() => chooseSubject(option.label)}><span className={styles.choiceIcon}>{option.icon}</span><span className={styles.choiceLabel}>{option.label}</span></button>)}</div><button className={styles.backButton} type="button" onClick={() => showScreen("intro")}>← Zurück</button></section> : null}

        {screen === "quiz" && activeQuestion && subject ? <section className={`${styles.screen} ${styles.quizScreen}`} aria-labelledby="quiz-question"><h2 className={`${styles.question} ${styles.quizTitle}`} id="quiz-question">{activeQuestion.question}</h2><div className={styles.answers}>{activeQuestion.options.map((option, index) => { const isCorrect = hasAnswered && index === activeQuestion.correctIndex; const isIncorrect = hasAnswered && index === selectedAnswer && !isCorrect; return <button className={`${styles.answer} ${isCorrect ? styles.answerCorrect : ""} ${isIncorrect ? styles.answerIncorrect : ""}`} type="button" key={option} disabled={hasAnswered} onClick={() => chooseAnswer(index)}><span className={styles.answerLetter}>{String.fromCharCode(65 + index)}</span><span>{option}</span></button>; })}</div>{hasAnswered ? <div className={styles.feedback}><p>{selectedAnswer === activeQuestion.correctIndex ? "Richtig." : `Fast – richtig ist: ${activeQuestion.options[activeQuestion.correctIndex]}.`}</p><span>{activeQuestion.explanation}</span></div> : null}{hasAnswered ? <button ref={continueButtonRef} className={`${styles.primaryButton} ${styles.quizNext}`} type="button" onClick={nextQuestion}>Zu deinem Ergebnis</button> : null}<button className={styles.backButton} type="button" onClick={() => showScreen("subject")}>← Fach ändern</button></section> : null}

        {screen === "result" && subject ? <section className={`${styles.screen} ${styles.centerScreen} ${styles.ctaScreen}`} aria-label="Medula Premium"><h2 className={styles.ctaTitle}>Mache jetzt in der Medula App weiter…</h2>{isSubmitted ? <div className={styles.ctaConfirmation} role="status"><div className={styles.confirmationIcon}>✓</div><h2>{position ? `Du bist auf Platz #${position}.` : "Du bist auf der Liste."}</h2><p>{rewardUnlocked ? "Lifetime Premium ist freigeschaltet." : `Lade ${referralGoal} Kommiliton:innen ein und sichere dir Lifetime Premium.`}</p>{referralLink ? <><p className={styles.referralProgress}>{Math.min(referralCount, referralGoal)}/{referralGoal} erfolgreiche Einladungen</p><div className={styles.referralRow}><input value={referralLink} readOnly aria-label="Persönlicher Einladungslink" /><button type="button" onClick={copyReferralLink}>{hasCopiedReferralLink ? "Kopiert" : "Kopieren"}</button></div><button className={styles.shareButton} type="button" onClick={shareReferralLink}>Mit Kommiliton:innen teilen</button></> : null}</div> : <><form className={styles.ctaForm} id="premium-signup" onSubmit={handleWaitlistSubmit}><label className={styles.visuallyHidden} htmlFor="email">E-Mail für den Launch</label><input className={styles.ctaEmailInput} id="email" name="email" type="email" autoComplete="email" autoCapitalize="none" autoCorrect="off" spellCheck={false} placeholder="deine@email.de" required onChange={() => recordQuizVisit("email_started", { subject, score })} /><input className={styles.honeypot} name="website" type="text" tabIndex={-1} autoComplete="off" />{errorMessage ? <p className={styles.ctaError} role="alert">{errorMessage}</p> : null}</form><button className={styles.ctaButton} form="premium-signup" type="submit" disabled={isSubmitting}>{isSubmitting ? "Wird gespeichert..." : "3 Monate Premium sichern"}</button><p className={styles.ctaAvailability}>Noch 12 Plätze frei</p></>}</section> : null}
      </main>
      {screen !== "intro" ? <footer className={styles.footer}><strong>Medula</strong> · Schnelle Wiederholung zwischen Uni, Station und Klausurphase</footer> : null}
    </div>
  );
}
